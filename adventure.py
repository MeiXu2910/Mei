import json
import sys
import difflib  # For finding closest matches for ambiguous commands

class GameEngine:
    def __init__(self, map_filename):
        self.load_map(map_filename)
        self.current_room = self.start_room
        self.inventory = []

    def load_map(self, map_filename):
        try:
            with open(map_filename, 'r') as f:
                map_data = json.load(f)
                # print('map data', map_data)
                self.start_room = map_data["start"]
                # print('start room', self.start_room)
                self.rooms = {room["name"]: room for room in map_data["rooms"]}  # Lowercase room names for case-insensitive matching
                # print('rooms', self.rooms)
        except FileNotFoundError:
            print("Error: Map file not found.")
            sys.exit(1)
        except json.JSONDecodeError:
            print("Error: Invalid map file format.")
            sys.exit(1)

    def look(self):
        room = self.rooms[self.current_room]
        print(f"> {room['name']}\n\n{room['desc']}\n")
        
        self.print_items(room)
        self.print_exits(room)

    def print_exits(self, room):
        print("Exits:", " ".join(room["exits"].keys())+ "\n")

    def print_items(self, room):
        if "items" in room:
            print("Items:", " ".join(room["items"])+ "\n")

    def go(self, direction):
        room = self.rooms[self.current_room]
        exits = room["exits"]
        
        if direction in exits:
            next_room_name = exits[direction]
            if self.is_door_locked(room, direction):
                print("The door is locked.")
            else:
                print(f'You go {direction}.\n')
                self.current_room = next_room_name
                self.look()
        else:
            closest_match = self.find_closest_exit(direction, exits)
            if closest_match:
                print(f"Did you mean to go {closest_match}?")
            else:
                print("There's no way to go", direction + ".")

    def find_closest_exit(self, direction, exits):
        closest_match = None
        closest_distance = float('inf')

        for exit_direction in exits:
            distance = self.calculate_similarity(exit_direction, direction)
            if distance < closest_distance:
                closest_match = exit_direction
                closest_distance = distance

        return closest_match
    def calculate_similarity(self, s1, s2):
        len_diff = abs(len(s1) - len(s2))  # 计算长度差异
        min_len = min(len(s1), len(s2))     # 获取较短字符串的长度
        common_prefix_len = 0               # 计算公共前缀长度

        # 计算公共前缀长度
        for i in range(min_len):
            if s1[i] == s2[i]:
                common_prefix_len += 1
            else:
                break

        # 返回相似度度量值，长度差异和公共前缀长度越小，相似度越高
        return len_diff + (min_len - common_prefix_len)

    def is_door_locked(self, room, direction):
        next_room_name = room["exits"][direction]
        next_room = self.rooms[next_room_name]
        return next_room.get("locked", False)

    def use(self, item):
        room = self.rooms[self.current_room]
        if item in self.inventory:
            exits = room["exits"]
            for direction, next_room_name in exits.items():
                next_room = self.rooms[next_room_name]
                if next_room.get("locked", False) and next_room.get("key") == item:
                    next_room["locked"] = False
                    print("You used", item, "to unlock the door to the", next_room["name"])
                    return
            print("You can't use", item, "here.")
        else:
            print("You don't have", item, "in your inventory.")

    def get(self, item):
        room = self.rooms[self.current_room]
        if "items" in room and item in room["items"]:
            room["items"].remove(item)
            self.inventory.append(item)
            print("You picked up", item)
        else:
            print("There's no", item, "here.")

    def drop(self, item):
        if item in self.inventory:
            self.inventory.remove(item)
            room = self.rooms[self.current_room]
            room["items"].append(item)
            print("You dropped", item)
        else:
            print("You don't have", item, "in your inventory.")

    def show_inventory(self):
        if self.inventory:
            print("Inventory:", ", ".join(self.inventory))
        else:
            print("Your inventory is empty.")

    def help(self):
        print("Available commands:")
        print("  go [direction]")
        print("  look")
        print("  get [item]")
        print("  drop [item]")
        print("  use [item]")
        print("  inventory")
        print("  quit\n")

    def quit(self):
        print("Goodbye!")
        sys.exit()

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 adventure.py [map filename]")
        sys.exit(1)

    game = GameEngine(sys.argv[1])
    game.look()

    while True:
        command = input("What would you like to do? ").strip().lower()
        if command == "quit":
            game.quit()
        elif command == "look":
            game.look()
        elif command.startswith("go"):
            try:
                _, direction = command.split(" ", 1)
                game.go(direction)
            except Exception:
        
                print("Sorry, you need to 'go' somewhere.")
        elif command.startswith("get"):
            _, item = command.split(" ", 1)
            game.get(item)
        elif command.startswith("drop"):
            _, item = command.split(" ", 1)
            game.drop(item)
        elif command.startswith("use"):
            _, item = command.split(" ", 1)
            game.use(item)
        elif command.startswith("inventory"):

            game.show_inventory()
        elif command.startswith("help"):
            game.help()